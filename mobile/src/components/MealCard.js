    ...
    <Card style={styles.card}>
      <View style={styles.row}>
        <View style={[styles.emojiCircle, { backgroundColor: colors.primarySoft, overflow: 'hidden' }]}>
          {meal.image ? (
            <Image source={typeof meal.image === 'string' ? { uri: meal.image } : meal.image} style={{ width: 44, height: 44, resizeMode: 'cover' }} />
          ) : (
        ...
        <View style={styles.calCol}>
          <Text style={[styles.cal, { color: colors.text }]}>{Number(meal.calories).toFixed(2)}</Text>
