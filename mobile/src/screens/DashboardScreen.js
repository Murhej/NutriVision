...
export default function DashboardScreen({ navigation }) {
      ...
      const fetchData = async () => {
        try {
          const [dashRes, profileRes] = await Promise.all([
            apiClient.get('/api/mobile/dashboard'),
            apiClient.get('/api/mobile/profile')
          ]);
          setData({
            nutrition: dashRes.nutrition,
            meals: dashRes.meals,
  ...
  const { nutrition: TODAY_NUTRITION, meals: TODAYS_MEALS, profile: USER_PROFILE } = data;
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        >
          {USER_PROFILE.avatar ? (
            <Image source={typeof USER_PROFILE.avatar === 'string' ? { uri: USER_PROFILE.avatar } : USER_PROFILE.avatar} style={{ width: 44, height: 44, borderRadius: 22 }} />
          ) : (
      ...
      <Card style={styles.calorieCard}>
        <View style={styles.calorieRow}>
          ...
          >
            <Text style={[styles.calNumber, { color: colors.text }]}>{Number(calories.consumed).toFixed(2)}</Text>
          ...
          <View style={styles.calorieInfo}>
            ...
            <Text style={[styles.calorieSubtitle, { color: colors.textSecondary }]}>
              {calories.goal - calories.consumed > 0
                ? `${Number(calories.goal - calories.consumed).toFixed(2)} kcal remaining`
                : 'Goal reached! 🎉'}
            </Text>
